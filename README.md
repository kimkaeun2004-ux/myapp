...
1. Copy env template and create local env file:
...
**Server (secret — no `NEXT_PUBLIC_` prefix)**

- `SUPABASE_URL` — same as project URL above
- `SUPABASE_SERVICE_ROLE_KEY` — Dashboard → Settings → API → `service_role` `secret`

3. Use clients from:

- `lib/supabase/client.ts` — browser / anon key
- `lib/supabase/server.ts` — API routes / server only / service role
